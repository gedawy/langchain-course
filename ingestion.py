...
async def async_extract(url_batches: List[List[str]]):
  ...
  log_info(f"🔧 TavilyExtract: Starting concurrent extraction of {len(url_batches)} batches", Colors.BLUE)
  ...
  for result in results:
    if isinstance(result, Exception):
      ...
      for extacted_page in result['results']: #type: ignore
        document = Document(
          page_content=extacted_page["raw_content"],
          metadata={"source": extacted_page["url"]}
        )
...
async def main():
  ...
  log_info(
      "🗺️  TavilyCrawl: Starting to map documentation structure from https://python.langchain.com/",
      Colors.PURPLE,
  )
  ...
  site_map = tavily_map.invoke("https://docs.langchain.com/oss/python/langchain/")
  ...
  log_info(f"✂ TextSplitter: Processing {len(all_docs)} documents with 4000 chunk size and 200 overlap", Colors.BLUE)

  text_splitter = RecursiveCharacterTextSplitter(chunk_size=4000, chunk_overlap=200)
  splitted_docs = text_splitter.split_documents(all_docs)
  log_success(f"TextSplitter: Created {len(splitted_docs)} chunks from {len(all_docs)} documents")
